...
var tempanswer = answers[currentlevel];

var hintsNum = 2; // number of hints
...
function main() {
	...
	var answer = answers[currentlevel];
	for (var i = 0; i < 26; ++i) {
	...
	// set onclick attribute of the images to viewimgFull
	// viewimgFull(defined in imagefullsize.js) expands the clicked image
	$(".hintimage").attr("onclick", "viewimgFull(this)");
...
function addimages(level) {
	...
	for (var i = 1; i <= 2; ++i) {
		...
		$("#imgSmall").append("<img src=" + imagelocation + " class='hintimage'>");
	}
	$("#imgSmall").append("<br>");
	for (var i = 3; i <= 4; ++i) {
		var imagelocation = dir + i + ".jpg" ;
...
function addblanks(level) {
	...
	$("#hintsNum")[0].innerHTML = hintsNum + " hint(s) remaining";
}
...
function nextmove() {
	if (checkifcorrect(currentlevel)) {
		...
		// show next level
		currentlevel++;
		tempanswer = answers[currentlevel];
...
function getRandomLetter() {
	...
	var answer = answers[currentlevel]; // get current level's answer
	for (var i = 0; i < answer.length; ++i) {
...
function hint() {
	...
	hintsNum--; // decrement number of hints by one
	addhint(letter, index, position); // add hint
	$("#hintsNum")[0].innerHTML = hintsNum + " hint(s) remaining"; // update the innerHTML for number of hints remaining
}
